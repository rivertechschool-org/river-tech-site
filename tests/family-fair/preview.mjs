// Local-only review server. Never deploy this process. Real Code.gs + fictional adapters.
...
http.createServer(async(req,res)=>{
 try{
  ...
  if(url.pathname==='/assets/js/family-fair-config.js'){res.setHeader('Content-Type','text/javascript');res.end("window.FAMILY_FAIR_API='/api/family-fair';");return;}
  ...
  if(target.endsWith('.html'))bytes=bytes.toString().replace('<body data-fair=', '<body data-fair=').replace(/(<body[^>]*>)/,'$1<div style="position:relative;z-index:101;background:#e7dcba;color:#3e3926;text-align:center;font:13px/1.5 system-ui;padding:6px">Private preview · fictional families · no emails sent · <a href="/__test/inbox">Test mailbox</a></div>');
  res.end(bytes);
