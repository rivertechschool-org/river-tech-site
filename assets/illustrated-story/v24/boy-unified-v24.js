...
const image=src=>new Promise((ok,no)=>{const im=new Image();im.onload=()=>ok(im);im.onerror=()=>no(Error('Boy artwork could not load'));im.src=new URL(src,import.meta.url).href});
export async function loadBoy(){const[data,body]=await Promise.all([fetch(new URL('motion-source/90_04-cartwheel.json',import.meta.url)).then(r=>r.json()),image('assets/story-v14/boy-launch-v8.webp')]);return{capture:prepareCapture(data,body),body,renderer:new PencilRenderer(body)}}
...
// Follow the jaw into the front of the neck. The old diagonal included a
// triangle of red shirt that rotated upward with the head during the skyward look.
const headHull=[[102,12],[156,12],[156,49],[146,50],[145,60],[131.5,65.8],[126.3,65.8],[125.5,68.5],[123,71.5],[115,69.5],[109,64.5],[112,56],[104,52],[102,40]];
function triangulatedHull(hull,maxEdge=5){
 const cross=(a,b,c)=>(b[0]-a[0])*(c[1]-a[1])-(b[1]-a[1])*(c[0]-a[0]);
 const area=hull.reduce((s,a,i)=>{const b=hull[(i+1)%hull.length];return s+a[0]*b[1]-b[0]*a[1]},0),sign=Math.sign(area),remaining=hull.map((_,i)=>i),base=[];
 while(remaining.length>3){let found=false;for(let k=0;k<remaining.length;k++){
  ...
  if(cross(hull[a],hull[b],hull[c])*sign<=0)continue;
  if(remaining.some(i=>i!==a&&i!==b&&i!==c&&cross(hull[a],hull[b],hull[i])*sign>=0&&cross(hull[b],hull[c],hull[i])*sign>=0&&cross(hull[c],hull[a],hull[i])*sign>=0))continue;
  base.push([hull[a],hull[b],hull[c]]);remaining.splice(k,1);found=true;break;
 }if(!found)throw Error('Character hull must be a simple polygon')}
 base.push(remaining.map(i=>hull[i]));
 const points=[],indices=[];
 const split=(a,b,c)=>{const d=[length(sub(a,b)),length(sub(b,c)),length(sub(c,a))],longest=Math.max(...d);if(longest<=maxEdge){const n=points.length;points.push(a,b,c);indices.push(n,n+1,n+2);return}
  if(d[0]===longest){const m=mix(a,b,.5);split(a,m,c);split(m,b,c)}else if(d[1]===longest){const m=mix(b,c,.5);split(a,b,m);split(a,m,c)}else{const m=mix(c,a,.5);split(a,b,m);split(m,b,c)}
 };base.forEach(t=>split(...t));return{points,indices};
}
const headMesh=triangulatedHull(headHull,Infinity);
const shoulderMeshes=[[[110,59],[114,65],[110,72],[102,81],[89,85],[85,78],[88,70],[99,64]],[[127,64],[136,66],[144,71],[147,81],[138,89],[131,79],[127,71]]].map(hull=>triangulatedHull(hull,Infinity));
...
export function drawBoy(ctx,art,P,{width,height,boyHeight,launch=false}){
 ...
 for(const side of(launch?['Right']:['Right','Left']))r.strip(arms[side],uv=>chain(uv,[side+'Arm',side+'ForeArm',side+'Hand',side+'Palm'],P,k));
 ...
 // Restore the source shoulder slopes between collar and sleeve. Keep the
 // accepted torso deformation; only this missing strip shares shoulder motion.
 for(let i=0;i<shoulderMeshes.length;i++){
  const panel=shoulderMeshes[i],side=i===0?'Left':'Right',delta=sub(P[side+'Arm'],torsoMap(T[side+'Arm']));
  r.mesh(panel.points,panel.indices,uv=>{
   const influence=(i===0?ease((108-uv[0])/15):ease((uv[0]-129)/14))*ease((uv[1]-65)/14);
   return add(torsoMap(uv),mul(delta,influence));
  });
 }
 ...
 // The near arm is one foreground limb, from red sleeve through fingertips.
 // Drawing only its forearm here hid the upper arm under the shirt on retraction.
 if(launch)r.strip(arms.Left,uv=>chain(uv,['LeftArm','LeftForeArm','LeftHand','LeftPalm'],P,k));
 r.mesh(headMesh.points,headMesh.indices,uv=>mapBone(uv,'Neck1','Head',P,k));
